...
template < class ConnKeyT >
__global__ void
setDelays( ConnKeyT* conn_key_subarray, float* arr_val, int64_t n_conn, float time_resolution, int max_allowed_delay, int *delayOverflow )
{
  int64_t i_conn = threadIdx.x + blockIdx.x * blockDim.x;
  if ( i_conn >= n_conn )
  {
...
ConnectionTemplate< ConnKeyT, ConnStructT >::setConnectionDelays( curandGenerator_t& gen,
  ...
  SynSpec& syn_spec )
{
  int max_allowed_delay = (1 << max_delay_nbits_) - 1;
  ...
  { // or array
    ...
    int *d_delayOverflow;
    CUDAMALLOCCTRL( "&d_delayOverflow", &d_delayOverflow, sizeof( int ) );
    gpuErrchk( cudaMemset( d_delayOverflow, 0, sizeof( int ) ) );
    setDelays< ConnKeyT > <<< ( n_conn + 1023 ) / 1024, 1024 >>>
      ( conn_key_subarray, ( float* ) d_storage, n_conn, time_resolution_, max_allowed_delay, d_delayOverflow );
    CUDASYNC;
    int h_delayOverflow;
    gpuErrchk( cudaMemcpy( &h_delayOverflow, d_delayOverflow, sizeof(int), cudaMemcpyDeviceToHost ) );
    CUDAFREECTRL( "d_delayOverflow", d_delayOverflow );
    if ( h_delayOverflow ) {
